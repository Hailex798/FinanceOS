...
- [ ] Each board card shows: board name, account name, total spend (sum of Expense cards only), net balance, and expense count.
...
- [ ] User can add an account with: name, bank name, account type (Savings, Current, Credit Card, Wallet), initial balance, color, icon.
...
- [ ] Each column shows its total amount at the top (Total Spend counts Expense cards only; Income is tracked separately for net balance).
...
- [ ] Expense card fields:
  ...
  - **Type** (required: Income or Expense, defaults to Expense)
  - **Amount** (required, positive number. If Income: adds to net balance; if Expense: subtracts from net balance. Entered in rupees — stored in paise)
  - **Category** (required, dropdown from user's categories)
  - **Date** (required, defaults to today)
  - **Notes** (optional, max 500 chars)
  - **Tax type** (optional, defaults to None: GST, TDS, Custom, None)
  - **Tax rate** (%, defaults to 0% — ignored if no tax)
  - **Tax amount** (auto-calculated for reporting: `amount × taxRate / 100`, displayed read-only. The main 'Amount' field remains the final total)
...
- [ ] User can mark a reminder as "Paid" (creates a new instance of the expense in the "Spent" column, keeping the original in the "Recurring" column as a template).
- [ ] UI matches the Stitch design system in layout, spacing, color, and component hierarchy. Verified against `DESIGN.md` tokens.
